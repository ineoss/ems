<template>
<div class="buyhistorydetail-wrapper">
	<mini-bar title="购置详情" back="true"></mini-bar>
	<list class="main">
		...
		<div class="row">
			<text class="label">实际购置价格：</text>
			<text class="input">{{plan.rPrice}}</text>
		</div>
		<div class="row">
			<text class="label">实际购置数量：</text>
			<text class="input">{{plan.rNum}}</text>
		</div>
		<div class="row">
			<text class="label">实际购置日期：</text>
			<text class="input">{{plan.rDate}}</text>
		</div>
		<div class="row">
			<text class="label">处理人：</text>
			<text class="input">{{plan.handler}}</text>
		</div>
		<div class="button-group">
			<button><text @click="delAlert" class="util" style="background-color:rgba(255,0,0,0.5)">删除</text></button>
		</div>
		</cell>
		<alert ref="delAlert" @dialogConfirmBtnClick="del" :single="false"></alert>
...
export default {
	...
	methods: {
		onrefresh(event) {
			...
			this.get('/getBuyInfo', { id: this.plan.id }, (result) => {
		...
		del() {
			this.post('/delBuy', { id: this.plan.id }, (result) => {
				if (result && result.state == 'success') {
	...
	created() {
		...
		if (params && params.id) {
			this.get('/getBuyInfo', { id: params.id }, (result) => {
				if (result && result.state == 'success') {
					this.plan = result.data
				}
			})

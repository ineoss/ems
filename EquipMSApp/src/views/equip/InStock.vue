...
<div class="instock-wrapper">
	<mini-bar title="设备入库" back="true" scan="true" @scanResult="scanResult"></mini-bar>
  <div class="main">
    <div class="row">
			<text class="label" >资产编号：</text>
			<input class="input" type="text" v-model="instock.assetNo" autofocus="true">
			<text class="required">*</text>
		</div>
    <div class="row">
			<text class="label">入库部门：</text>
			<button @click="showSelectList('deptlist')" class="input" style="justify-content:center"><text>{{deptname}}</text></button>
      <text class="required">*</text>
		</div>
    <div class="row">
			<text class="label">备注：</text>
			<textarea class="textarea" rows="6" v-model="instock.mark"></textarea>
		</div>
...
export default {
  ...
  methods: {
    ...
    submit() {
      if (this.checkFormValue(this.instock.assetNo, '资产编号不能为空')
        && this.checkFormValue(this.instock.instockDept, '请选择入库部门')) {
        this.post("/inStock", this.instock, (result) => {
          if (result && result.state == 'success') {
            this.$refs.alert.show("入库成功")
            this.instock.assetNo = ''
          } else if (result && result.state == 'fail') {
            this.$refs.alert.show(result.data)
          } else {
            this.$refs.alert.show("入库失败")
          }
        })
      }
    },
